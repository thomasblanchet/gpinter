#' @title Peano kernel for the interpolation method with known derivatives
...
central_peano_kernel_degree3 <- function(x, t, x0, x1, x2, x3) {
...
left_peano_kernel_degree3 <- function(x, t, x0, x1, x2) {
    # Parameters of the spline for the Peano kernel
    y0 <- 0
    y1 <- (x1 - t)^3
    s0 <- 0
...
right_peano_kernel_degree3 <- function(x, t, x0, x1, x2) {
    # Parameters of the spline for the Peano kernel
    y1 <- 0
    y2 <- (x2 - t)^3
    s0 <- 0
...
#' @title Spline error for the central intervals with known derivatives
#'
#' @author Thomas Blanchet
#'
...
central_spline_error_degree3 <- function(x, x0, x1, x2, x3, upperbound) {
    return(integrate(function(t) {
        abs(central_peano_kernel_degree3(x, t, x0, x1, x2, x3)*upperbound(t))/6
...
left_spline_error_degree3 <- function(x, x0, x1, x2, upperbound) {
    return(integrate(function(t) {
        abs(left_peano_kernel_degree3(x, t, x0, x1, x2)*upperbound(t))/6
    }, lower=x0, upper=x1)$value)
}

#' @title Spline error for the right interval with known derivatives
#'
#' @author Thomas Blanchet
#'
...
right_spline_error_degree3 <- function(x, x0, x1, x2, upperbound) {
    return(integrate(function(t) {
        abs(right_peano_kernel_degree3(x, t, x0, x1, x2)*upperbound(t))/6
    }, lower=x1, upper=x2)$value)
}
...
spline_error_degree3 <- function(x, xk, upperbound) {
    n <- length(xk)
    ...
    return(sapply(1:length(x), function(i) {
        if (is.na(k[i]) | k[i] == n) {
            ...
            return(left_spline_error_degree3(x[i], xk[1], xk[2], xk[3], upperbound))
        } else if (k[i] == (n - 1)) {
            return(right_spline_error_degree3(x[i], xk[n - 2], xk[n - 1], xk[n], upperbound))
        } else {
            return(central_spline_error_degree3(x[i], xk[k[i] - 1], xk[k[i]], xk[k[i] + 1], xk[k[i] + 2], upperbound))
        }
    }))
}
