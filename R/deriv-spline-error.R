...
deriv_central_peano_kernel <- function(x, t, x0, x1, x2, x3) {
...
deriv_left_peano_kernel <- function(x, t, x0, x1, x2) {
    # Parameters of the spline for the Peano kernel
    y0 <- 0
    y1 <- (x1 - t)^3
    s0 <- 0
...
deriv_right_peano_kernel <- function(x, t, x0, x1, x2) {
    # Parameters of the spline for the Peano kernel
    y1 <- 0
    y2 <- (x2 - t)^3
    s0 <- 0
...
#' @title Shape of interpolation error of the derivative for the central
#' intervals
#'
#' @author Thomas Blanchet
#'
...
deriv_central_spline_error <- function(x, x0, x1, x2, x3, upperbound) {
    return(integrate(function(t) {
        abs(deriv_central_peano_kernel(x, t, x0, x1, x2, x3)*upperbound(t))/6
    }, lower=x1, upper=x2)$value)
}

#' @title Shape of interpolation error of the derivative for the left interval
#'
#' @author Thomas Blanchet
#'
#' @description Gives the bound on the interpolation error of the
...
deriv_left_spline_error <- function(x, x0, x1, x2, upperbound) {
    return(integrate(function(t) {
        abs(deriv_left_peano_kernel(x, t, x0, x1, x2)*upperbound(t))/6
    }, lower=x0, upper=x1)$value)
}

#' @title Shape of interpolation error of the derivative for the right interval
#'
#' @author Thomas Blanchet
#'
#' @description Gives the bound on the interpolation error of the
...
deriv_right_spline_error <- function(x, x0, x1, x2, upperbound) {
    return(integrate(function(t) {
        abs(deriv_right_peano_kernel(x, t, x0, x1, x2)*upperbound(t))/6
    }, lower=x1, upper=x2)$value)
}
...
deriv_spline_error <- function(x, xk, upperbound) {
    n <- length(xk)
    ...
    return(sapply(1:length(x), function(i) {
        if (is.na(k[i]) | k[i] == n) {
            ...
            return(deriv_left_spline_error(x[i], xk[1], xk[2], xk[3], upperbound))
        } else if (k[i] == (n - 1)) {
            return(deriv_right_spline_error(x[i], xk[n - 2], xk[n - 1], xk[n], upperbound))
        } else {
            return(deriv_central_spline_error(x[i], xk[k[i] - 1], xk[k[i]], xk[k[i] + 1], xk[k[i] + 2], upperbound))
        }
    }))
}
